---
title: 工具
description: Lead Agent 是一个工具调用 Agent。工具是它与工作区交互的方式：读写文件、运行命令、委派任务以及向用户呈现输出。
---
...
Lead Agent 是一个工具调用 Agent。工具是它与工作区交互的方式：读写文件、运行命令、委派任务以及向用户呈现输出。

DeerFlow 将工具分为三类：

1. **内置工具** — 核心运行时能力，始终对 Agent 可用
2. **MCP 工具** — 由外部 Model Context Protocol 服务器提供的工具
3. **技能工具** — 与特定技能包捆绑的工具
...
自定义 Agent 然后可以在其配置中按名称引用组，将其工具访问限制为仅相关集合。DeerFlow 当前默认不再内置外部网页搜索、网页抓取和图像搜索能力。
