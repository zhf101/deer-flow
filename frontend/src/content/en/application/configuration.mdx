...
Configure which tools the agent has access to. The default app setup focuses on workspace operations plus any MCP tools you enable separately:
...
If you need external capabilities, add them through MCP servers in `extensions_config.json` rather than built-in web search or scraping providers.
