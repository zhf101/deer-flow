...
class OracleAdapter:
    ...
    def _load_driver(self):
        try:
            ...
            raise DatabaseConnectionError(
                "Oracle driver is not installed. Add the 'oracledb' package to enable Oracle nlp2sql support."
            ) from exc

    def _build_dsn(self, driver) -> str:
        port = self.config.port or 1521
        if self.config.sid:
            return driver.makedsn(self.config.host, port, sid=self.config.sid)
        service_name = self.config.service_name or self.config.database
        return driver.makedsn(self.config.host, port, service_name=service_name)
    ...
    def connect(self) -> None:
        ...
        self._driver = driver
        try:
            if self.config.oracle_client_path and not type(self)._thick_mode_initialized:
                driver.init_oracle_client(lib_dir=self.config.oracle_client_path)
                type(self)._thick_mode_initialized = True
        ...
        try:
            self._conn = driver.connect(
                ...
                dsn=self._build_dsn(driver),
                tcp_connect_timeout=self.config.connect_timeout_seconds,
            )
            if hasattr(self._conn, "autocommit"):
                self._conn.autocommit = True
            if hasattr(self._conn, "call_timeout"):
                self._conn.call_timeout = self.config.query_timeout_seconds * 1000
        except Exception as exc:
    ...
    def _cursor(self):
        if self._conn is None:
            raise DatabaseConnectionError("Oracle connection is not initialized")
    ...
    def _current_owner(self) -> str:
        ...
        rows = self._fetch_all("SELECT USER AS CURRENT_SCHEMA FROM DUAL")
            ...
            columns_by_table.setdefault(key, []).append(
                {
                    ...
                    "column_type": _format_oracle_type(row),
...
def _format_oracle_type(row: dict[str, Any]) -> str:
    ...
    if data_type == "NUMBER":
        ...
        return data_type
    if data_type in {"VARCHAR2", "CHAR", "NCHAR", "NVARCHAR2"} and data_length:
