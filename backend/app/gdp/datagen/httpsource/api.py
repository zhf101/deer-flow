...
def _get_service() -> HttpSourceService:
    ...
    return HttpSourceService(HttpSourceRepository(sf))
...
async def list_http_sources(service: HttpSourceService = Depends(_get_service)) -> list[HttpSourceResponse]:
    return await service.list_http_sources()
...
async def get_http_source(sourceCode: str, service: HttpSourceService = Depends(_get_service)) -> HttpSourceResponse:
