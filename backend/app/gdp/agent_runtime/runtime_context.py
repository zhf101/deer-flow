"""GDP Agent Runtime 运行上下文工具。

本模块承接运行上下文能力，但不依赖旧 GDP Agent 核心目录，
避免新 Runtime 回连旧 graph 核心。
"""
...
def build_gdp_runtime_context(config: RunnableConfig | None, metadata: Any | None = None) -> dict[str, Any]:
    """从 RunnableConfig 和装配元数据生成 checkpoint 里的轻量运行上下文。"""
